...
test("rename should rename the keys in given object", (assert) => {
...
test("rename should ignore object`s properties without match", (assert) => {
...
test("rename should ignore key`s properties without match", (assert) => {
  const object1 = { created_at: "2000-01-01", title: "Hello world!" }
...
test("dig returns the value of a field", (assert) => {
  assert.deepEqual(object.dig({ foo: "bar" }, "foo"), "bar")
})

test("dig returns the value of a nested field", (assert) => {
  assert.deepEqual(object.dig({ foo: { bar: "baz" } }, "foo.bar"), "baz")
})

test("dig returns null if the key does not match", (assert) => {
  assert.deepEqual(object.dig({ foo: "bar" }, "baz"), null)
})

test("pat overrides a value of a field", (assert) => {
  assert.deepEqual(object.pat({ foo: "bar" }, "foo", "baz"), { foo: "baz" })
})

test("pat overrides deeply nested fields", (assert) => {
  assert.deepEqual(object.pat({}, "baz.bar", "foo"), { baz: { bar: "foo" } })
})

test("values returns an array containing the values that correspond to all of a given object's own enumerable string properties", (assert) => {
  const parsed = object.values({ foo: "bar", baz: [1, 2, 3] })
  assert.deepEqual(parsed.length, 2)
})

test("values returns an empty array when object doesn't have own enumerable string properties", (assert) => {
  const parsed = object.values({})
  assert.deepEqual(parsed.length, 0)
})

test("keys returns an array containing the names of all of the given object's own enumerable string properties", (assert) => {
  const parsed = object.keys({ foo: "bar", baz: [1, 2, 3] })
  assert.deepEqual(parsed.length, 2)
})

test("keys returns an empty array when object doesn't have own enumerable string properties", (assert) => {
  const parsed = object.keys({})
  assert.deepEqual(parsed.length, 0)
})

test("merge merges properties of source into the destination object", (assert) => {
...
test("merge merges properties of sources into the destination object", (assert) => {
  const destination = { a: 1, b: 1, c: 1 }
...
test("clone creates shallow copy of object", (assert) => {
...
test("deepclone creates deep copy of object", (assert) => {
  const source = { a: 1, b: { foo: "bar" } }
...
test("recsort sorts keys of objects recursively", (assert) => {
